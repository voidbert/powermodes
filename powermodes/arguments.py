...
This module handles parsing and validation of command-line arguments. These are described in the
project's README, or by running ``powermodes --help``.

To handle command-line arguments, you're likely looking for the :func:`parse_validate_arguments`
method. If you wish to better control each aspect of the handling of command-line arguments, see
:func:`parse_arguments` and :func:`validate_arguments`. Each of this methods is accompanied by
...
class Arguments:
    ...
    If this data structure has been generated by :func:`validate_arguments`
    (or :func:`parse_validate_arguments`), there are some safe assumptions you can make, if that
    method reported no fatal errors (:attr:`~powermodes.error.ErrorType.ERROR`):

    - If ``action in [Action.INTERACTIVE, Action.APPLY_MODE, Action.VALIDATE]``, then
      :attr:`config` is not :data:`None`;
    - If ``action == Action.APPLY_MODE``, then :attr:`mode` is not :data:`None`.
    """
...
def parse_arguments(args: Optional[list[str]] = None) -> \
    ...
    :return: An ``argparse.Namespace`` with the following variables:
            ...
            In case of parsing error, :data:`None` will be returned with a single error.

    Examples:

    .. code:: python

        # Keep in mind that handle_error is not to be used in non-user-facing code.

        >>> handle_error(parse_arguments(['powermodes']))
        Namespace(actions=None, config=None, mode=None)
        # Action.SHOW_HELP is set by validate_arguments().

        >>> handle_error(parse_arguments(['powermodes', '--help', '-h']))
        Namespace(actions=[Action.SHOW_HELP, Action.SHOW_HELP], config=None, mode=None)

        >>> handle_error(parse_arguments(['powermodes', '-h', '-v', '-i']))
        Namespace(actions=[Action.SHOW_HELP, Action.VALIDATE, Action.INTERACTIVE ], \\
            config=None, mode=None)

        >>> handle_error(parse_arguments(['powermodes', '-c', '1.toml', '-c', '2.toml']))
        Namespace(actions=None, config=['1.toml', '2.toml'], mode=None)

        >>> handle_error(parse_arguments(['powermodes', '-m' , 'powersave']))
        Namespace(actions=None, config=None, mode=['powersave'])
        # Action.APPLY_MODE is set by validate_arguments().
    """
...
def __get_action(parsed_args: Namespace) -> tuple[Optional[Action], Optional[Error]]:
    ...
    This function is also responsible for considering the action to be :attr:`Action.SHOW_HELP`
    when no action is specified, and :attr:`Action.APPLY_MODE` when at least a powermode is
    specified.
...
def validate_arguments(parsed_args: Namespace) -> tuple[Optional[Arguments], list[Error]]:
    ...
    These are the argument errors and warnings this method can report:

    - Multiple actions specified, a fatal error, because powermodes doesn't know what to do;

    - A warning for single action specified multiple times, e.g.: ``powermodes --help -h``;

    - Warnings for multiple specified configuration files (the last one is chosen), or an
      unnecessarily specified configuration file, e.g.: ``powermodes --help -c config.toml``;

    - An error if a configuration file isn't provided for an action that requires it;

    - A warning for multiple specified powermodes to apply (the last one is chosen).

    :param parsed_args: Parsed command-line arguments (see :func:`parse_arguments`).
    :return: Organized command-line argument information (or :data:`None`, on failure), along with
             possible errors and warnings. See some certainties about the returned value (on
             success) in :class:`Arguments`.

    Example:

    This method is supposed to be used with the output from :func:`parse_arguments`. See the source
    code of :func:`parse_validate_arguments` for an idea on how to do so. Also, see
    :func:`parse_validate_arguments`'s examples for examples on how arguments get transformed into
...
def parse_validate_arguments(args: Optional[list[str]] = None) -> \
    tuple[Optional[Arguments], list[Error]]:
    """Parses and, on success of the parsing, validates command-line arguments. To understand each
    step of this function, see :func:`parse_arguments`, followed by :func:`validate_arguments`:.
    ...
    .. code:: python
        ...
        >>> handle_error(parse_validate_arguments(['powermodes']))
        Arguments(action=Action.SHOW_HELP, config=None, mode=None)

        >>> handle_error(parse_validate_arguments(['powermodes', '-m', 'powersave']))
        error: No config file specified.
        Arguments(action=Action.APPLY_MODE, config=None, mode='powersave')

        >>> handle_error(parse_validate_arguments(['powermodes', '-vc', 'config.toml']))
        Arguments(action=Action.VALIDATE, config='config.toml', mode=None)
    """

    errors: list[Error] = []
    parsed = handle_error_append(errors, parse_arguments(args))
    if parsed is None:
        return (None, errors)
    validated = handle_error_append(errors, validate_arguments(parsed))
...
def get_version_string() -> tuple[Optional[str], Optional[Error]]:
    ...
    :return: The version of powermodes along with an empty error list, or :data:`None` and an
             error (``importlib.metadata.version`` failed).
    """
